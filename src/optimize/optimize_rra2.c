...
/*   Updated: 2022/06/30 01:10:06 by mayocorn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/optimize.h"

static void	optimize_ra_rra_sub(t_deque *commands, t_node **node, \
...
void	optimize_ra_rra(t_deque *commands, t_node **node)
{
	t_command	command;
	t_node		*now;
	t_node		*pab_node;
	...
	while (now != NULL)
	{
		...
		if (command == ra)
			return (optimize_ra_rra_sub(commands, node, now, pab_node));
		else if (pab_node == NULL && (command == pa || command == pb))
			pab_node = now;
		else if (command == sa || command == pa || command == pb || command == rra)
			break ;
		now = now -> prev;
	}
	*node = (*node) -> next;
