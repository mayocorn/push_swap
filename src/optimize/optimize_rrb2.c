...
/*   Updated: 2022/06/29 19:27:04 by mayocorn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	optimize_rb_rrb(t_deque *commands, t_node **node)
{
	t_command	command;
	t_node		*now;
	t_node		*pab_node;
	...
	while (now != NULL)
	{
		...
		if (command == rb)
			return (optimize_rb_rrb_sub(commands, node, now, pab_node));
		else if (pab_node == NULL && (command == pa || command == pb))
			pab_node = now;
		else if (command == sb || command == pa || command == pb || command == rrb)
			break ;
		now = now -> prev;
	}
	*node = (*node) -> next;
