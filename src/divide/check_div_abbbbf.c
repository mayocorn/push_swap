...
/*   Updated: 2022/07/08 00:53:06 by mayocorn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_div_abbbbf(t_div_info *div_info, size_t *max_cnt, int min_element)
{
	size_t	cnt;
	size_t	size;
	int		large;
	...
	large = size - (size + 1) / 3 + min_element;
...
static size_t	cnt_rr(t_div_info *div_info, int large, int small)
{
	...
	while (node != NULL)
	{
		if (node->content.number >= large && hold_cnt)
		{
			...
			hold_cnt = 0;
		else if (node->content.number > small && node->content.number < large)
			hold_cnt++;
		node = node->next;
	}
	return (cnt);
